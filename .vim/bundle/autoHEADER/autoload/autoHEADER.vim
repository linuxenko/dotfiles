...
"     Last Modified       :     [2018-02-25 13:49]
...
let s:style_list = [
            \    { 'style' : ['/*' , '*' , '*/' , '*' ] , 'ft' : [ 'c' , 'java' , 'cpp' , 'javascript.jsx' , 'go' , 'php' , 'jsp' ] , 
            \      'prefix' : {
            \                 'php' : '<?php',
            \                 'jsp' : '<%',
            \                 },
    ...
    for styledict in s:style_list
        ...
        if indexofft >= 0
            ...
            for message in messages
                call append(start_line, start_char . repeat(' ',1) . printf('%-14s: %s',get(message,0),get(message,1)))
                let start_line += 1
            endfor
